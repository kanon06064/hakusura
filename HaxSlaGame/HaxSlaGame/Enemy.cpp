...
Enemy::Enemy(Vector3 startPos, EnemyData d, int floorLevel) {
    position = startPos; data = d; enemy_type = (EnemyType)d.type; state = STATE_PATROL; level = floorLevel;
...
void Enemy::Update(Player& p, Dungeon& d, std::vector<Projectile>& proj) {
    ...
    if (state == STATE_CHASE || state == STATE_ATTACK) {
        ...
        if (dist < attackRange && attackTimer <= 0) {
            ...
            if (enemy_type == E_ARCHER) { proj.push_back(Projectile{ position, Vector3Scale(lastAttackDir, 18), 0.2f, true, 0 }); attackTimer = 1.8f; }
            else if (enemy_type == E_MAGE) { proj.push_back(Projectile{ position, Vector3Scale(lastAttackDir, 8), 0.6f, true, 1 }); attackTimer = 2.2f; }
            else {
                float k = (enemy_type == E_SPEAR) ? 1.5f : 1.0f;
...
void Enemy::Draw() {
    Color c = (enemy_type == E_SWORD) ? MAROON : (enemy_type == E_SPEAR) ? ORANGE : (enemy_type == E_AXE) ? PURPLE : (enemy_type == E_ARCHER) ? DARKBLUE : (enemy_type == E_MAGE) ? PINK : GRAY;
    float h = (enemy_type == E_TRAP ? 2.0f : 1.2f);
    DrawCube(position, 1.0f, h, 1.0f, c); DrawCubeWires(position, 1.0f, h, 1.0f, BLACK);
    if (visualTimer > 0) {
        float b = atan2f(lastAttackDir.z, lastAttackDir.x);
        if (enemy_type == E_SWORD) { for (int i = -60; i <= 60; i += 15) DrawLine3D(position, Vector3Add(position, Vector3Scale(Vector3{ cosf(b + (float)i * DEG2RAD), 0, sinf(b + (float)i * DEG2RAD) }, 2.5f)), RED); }
        else if (enemy_type == E_AXE) DrawSphereWires(Vector3Add(position, Vector3Scale(lastAttackDir, 2)), 2.5f, 8, 8, RED);
        else if (enemy_type == E_SPEAR) {
