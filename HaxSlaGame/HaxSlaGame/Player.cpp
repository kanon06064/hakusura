...
void Player::PerformAttack(Vector3 ad, std::vector<Enemy>& enemies, Dungeon& d, std::vector<DamageText>& dt) {
    if (currentWeapon == BOW) projectiles.push_back({ position, Vector3Scale(ad, 25), 0.2f, true, 0 });
        ...
        for (auto& e : enemies) {
            ...
            case SPEAR: { float f = Vector3DotProduct(v, ad), s = fabsf(Vector3DotProduct(v, { -ad.z,0,ad.x })); if (f > 0 && f < 8.2f && s < 1.3f) { hit = true; knk = 2.2f; mult = 1.2f; } } break;
...
void Player::Draw() {
    ...
    if (isAttacking) {
        ...
        if (currentWeapon == SWORD) { for (int i = -60; i <= 60; i += 10) DrawLine3D({ position.x, 0.1f, position.z }, { position.x + cosf(b + i * DEG2RAD) * 4.5f, 0.1f, position.z + sinf(b + i * DEG2RAD) * 4.5f }, YELLOW); }
        else if (currentWeapon == SPEAR) { Vector3 r = { -lastAimDir.z,0,lastAimDir.x }, p1 = Vector3Add(position, Vector3Scale(r, 1.3f)), p2 = Vector3Subtract(position, Vector3Scale(r, 1.3f)), p3 = Vector3Add(p2, Vector3Scale(lastAimDir, 8.2f)), p4 = Vector3Add(p1, Vector3Scale(lastAimDir, 8.2f)); p1.y = p2.y = p3.y = p4.y = 0.1f; DrawLine3D(p1, p2, YELLOW); DrawLine3D(p2, p3, YELLOW); DrawLine3D(p3, p4, YELLOW); DrawLine3D(p4, p1, YELLOW); }
