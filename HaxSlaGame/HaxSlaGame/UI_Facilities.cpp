...
void UI::DrawCraftingMenu(Player& p, Font font, bool& isOpen) {
    ...
    DrawTextEx(font, T("CRAFT", "Craft").c_str(), { 80, 70 }, 24, 1, ORANGE);
    ...
    for (int i = 0; i < perPage; i++) {
        ...
        DrawTextEx(font, TextFormat(T("COST", "Cost: %d G").c_str(), r.cost), { (float)sw - 320, y + 25 }, 16, 1, (p.gold >= r.cost ? YELLOW : RED));
        ...
        if (canCraft && p.gold >= r.cost) {
            if (UI::DrawButton(createBtn, T("CREATE", "Create").c_str(), font, locked ? Fade(ORANGE, 0.5f) : ORANGE) && !locked) {
                ...
                for (auto& m : r.materials) {
                    for (auto it = p.inventoryItems.begin(); it != p.inventoryItems.end(); ) {
                        if (it->id == m.itemId) {
                            it->count -= m.count;
                            if (it->count <= 0) it = p.inventoryItems.erase(it);
                            else ++it;
                            break;
                        }
                        else ++it;
            ...
            DrawRectangle((int)sw - 200, (int)y + 15, 80, 40, showDetail ? ColorBrightness(GRAY, -0.4f) : GRAY);
        }
    }
...
void UI::DrawStorage(Player& p, Font font, bool& isOpen, std::vector<ItemData>& sItems, std::vector<ItemData>& sEquip) {
    ...
    for (int i = 0; i < perP; i++) {
        int idx = storageInvPage * perP + i;
        if (idx >= (int)p.inventoryItems.size()) break;

        Rectangle r = { leftX, (float)170 + i * 42, 350, 38 };
        ...
        if (UI::DrawButton({ leftX + 360, r.y, 110, 38 }, T("DEPOSIT", "Deposit >>").c_str(), font, modalLocked ? Fade(BLUE, 0.5f) : BLUE) && !modalLocked) {
            transferIdx = idx; isDeposit = true; transferAmount = 1; transferMax = p.inventoryItems[idx].count; AudioManager::PlaySE(SE_CLICK);
        }
    }
    ...
    for (int i = 0; i < perP; i++) {
        ...
        if (idx >= (int)sItems.size()) break;

        Rectangle r = { rightX, (float)170 + i * 42, 350, 38 };
        ...
        if (UI::DrawButton({ rightX - 120, r.y, 110, 38 }, T("WITHDRAW", "<< Withdraw").c_str(), font, modalLocked ? Fade(DARKGREEN, 0.5f) : DARKGREEN) && !modalLocked) {
            transferIdx = idx; isDeposit = false; transferAmount = 1; transferMax = sItems[idx].count; AudioManager::PlaySE(SE_CLICK);
        }
    }

    if (UI::DrawButton({ rightX, bottomY, 100, 30 }, "<<", font, modalLocked ? GRAY : GRAY) && !modalLocked && storageBoxPage > 0) storageBoxPage--;
    if (UI::DrawButton({ rightX + 110, bottomY, 100, 30 }, ">>", font, modalLocked ? GRAY : GRAY) && !modalLocked && storageBoxPage < mPBox - 1) storageBoxPage++;
    ...
    if (transferIdx != -1 && !showDetail) {
        ...
        if (UI::DrawButton({ (float)mx + 60, (float)my + 190, 120, 40 }, T("YES", "Confirm").c_str(), font, GREEN)) {
            if (isDeposit) {
                bool found = false;
                for (auto& si : sItems) {
                    if (si.id == targetItem.id) { si.count += transferAmount; found = true; break; }
                }
                if (!found) {
                    ItemData copy = targetItem; copy.count = transferAmount; sItems.push_back(copy);
                }
                ...
                bool found = false;
                for (auto& pi : p.inventoryItems) {
                    if (pi.id == targetItem.id) { pi.count += transferAmount; found = true; break; }
                }
                if (!found) {
                    if (p.inventoryItems.size() < MAX_ITEM_TYPES) {
                        ItemData copy = targetItem; copy.count = transferAmount; p.inventoryItems.push_back(copy);
                    }
                    else {
                        found = true;
                    }
        ...
        if (UI::DrawButton({ (float)mx + 240, (float)my + 190, 120, 40 }, T("NO", "Cancel").c_str(), font, RED)) {
            transferIdx = -1; AudioManager::PlaySE(SE_CLICK);
        }
    }
    DrawDetailWindow(font);
...
// ==========================================
// ワープポータル (解放済みのダンジョンと到達階層を選んで転送する)
// ==========================================
void UI::DrawWarpMenu(void* gamePtr, int unlockedDungeon, const std::vector<int>& maxFloors, Font font, bool& isOpen) {
    ...
    float boxW = 800.0f;
    float startX = centerX - boxW / 2.0f;

    DrawRectangle(startX, 50, boxW, sh - 100, Fade(BLACK, 0.95f));
    DrawRectangleLines(startX, 50, boxW, sh - 100, SKYBLUE);
    DrawTextEx(font, T("WARP_PORTAL", "Warp Portal").c_str(), { startX + 30, 70 }, 24, 1, WHITE);
    if (UI::DrawButton({ startX + boxW - 130, 65, 100, 40 }, T("CLOSE", "Close").c_str(), font, locked ? Fade(RED, 0.5f) : RED) && !locked) { isOpen = false; return; }
    ...
    for (int i = 0; i < 3; i++) {
        Rectangle tabR = { startX + 30.0f + i * 210, 130, 200, 40 };
    ...
    // ★修正: 変数名を maxF から maxWarpFloor に変更し、エラーを確実に回避します
    int maxWarpFloor = maxFloors[selectedDungeonTab];
    std::vector<int> warpFloors;

    for (int i = 5; i <= maxWarpFloor; i += 5) {
        warpFloors.push_back(i);
    }

    if (warpFloors.empty()) {
        DrawTextEx(font, T("NO_WARP", "No Warp Available").c_str(), { startX + 50, 250 }, 20, 1, GRAY);
    }
    else {
        const int perPage = 7;
        ...
        for (int i = 0; i < perPage; i++) {
            ...
            std::string label = TextFormat(T("FLOOR_INFO", "Floor %d").c_str(), f);
            if (f % 10 == 0) label += T("BOSS", " (Boss)");
            else if (f % 10 == 5) label += T("SAFE", " (Safe)");

            Rectangle r = { startX + 50.0f, 200.0f + i * 55, boxW - 100.0f, 45 };
        ...
        float bottomY = sh - 120.0f;
        if (UI::DrawButton({ startX + 50, bottomY, 100, 30 }, "<<", font, locked ? GRAY : GRAY) && !locked && warpScroll > 0) warpScroll--;
        if (UI::DrawButton({ startX + boxW - 150, bottomY, 100, 30 }, ">>", font, locked ? GRAY : GRAY) && !locked && warpScroll < maxP - 1) warpScroll++;
    }
}
...
void UI::DrawQuestMenu(Player& p, Font font, bool& isOpen) {
    ...
    for (int i = 0; i < perPage; i++) {
        ...
        int activeIdx = -1;
        for (int j = 0; j < (int)p.activeQuests.size(); j++) { if (p.activeQuests[j].questId == q.id) { activeIdx = j; break; } }

        Rectangle btnRect = { (float)sw - 220, y + 20, 100, 45 };

        if (isCleared) {
            DrawTextEx(font, T("CLEARED", "Cleared").c_str(), { (float)sw - 200, y + 30 }, 20, 1, GRAY);
        }
        else if (activeIdx != -1) {
            ...
            if (q.type == QUEST_HUNT) {
                ...
                if (pq.isCompleted) {
                    if (UI::DrawButton(btnRect, T("REPORT", "Report").c_str(), font, locked ? Fade(ORANGE, 0.5f) : ORANGE) && !locked) { p.CompleteQuest(q.id); AudioManager::PlaySE(SE_CLICK); }
                }
                ...
                for (const auto& it : p.inventoryItems) { if (it.id == q.targetId) currentInvCount += it.count; }
    ...
    float bottomY = sh - 100.0f;
    if (UI::DrawButton({ 80, bottomY, 100, 30 }, "<<", font, locked ? GRAY : GRAY) && !locked && questScroll > 0) questScroll--;
    if (UI::DrawButton({ 200, bottomY, 100, 30 }, ">>", font, locked ? GRAY : GRAY) && !locked && questScroll < maxP - 1) questScroll++;
}