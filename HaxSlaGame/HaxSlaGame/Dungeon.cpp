...
void Dungeon::Generate(bool homeMode) {
    ...
    stairsDownPos = { -999,-999,-999 }; stairsUpPos = { -999,-999,-999 }; storageBoxPos = { -999,-999,-999 };

    for (int y = 0; y < MAP_HEIGHT; y++)
        for (int x = 0; x < MAP_WIDTH; x++) { map[x][y] = 1; discovered[x][y] = homeMode; }

    if (homeMode) {
        Room c = { 15, 15, 10, 10 }; rooms.push_back(c);
        for (int ry = c.y; ry < c.y + c.height; ry++) for (int rx = c.x; rx < c.x + c.width; rx++) map[rx][ry] = 0;
        stairsDownPos = { 20 * TILE_SIZE, 0.1f, 22 * TILE_SIZE };
        storageBoxPos = { 17 * TILE_SIZE, 0.5f, 17 * TILE_SIZE };
    }
    else {
        for (int i = 0; i < 8; i++) {
        ...
        stairsUpPos = { (float)rooms[0].x * TILE_SIZE + 1.0f, 0.1f, (float)rooms[0].y * TILE_SIZE + 1.2f };
...
void Dungeon::UpdateVisibility(Vector3 p) {
    ...
    for (int y = gz - 5; y <= gz + 5; y++) for (int x = gx - 5; x <= gx + 5; x++) if (x >= 0 && x < MAP_WIDTH && y >= 0 && y < MAP_HEIGHT) discovered[x][y] = true;
}
