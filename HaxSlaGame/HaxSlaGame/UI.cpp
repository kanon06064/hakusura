...
void UI::DrawDetailWindow(Font font) {
    ...
    if (totalAtk != 0) { DrawTextEx(font, TextFormat("攻撃力 : %+.1f", totalAtk), { (float)x + 40, (float)statsY }, 22, 1, RED); if (mod.atk != 0) DrawTextEx(font, TextFormat("(補正 %+.1f)", mod.atk), { (float)x + 250, (float)statsY }, 18, 1, ORANGE); statsY += lineH; }
    if (totalDef != 0) { DrawTextEx(font, TextFormat("防御力 : %+.1f", totalDef), { (float)x + 40, (float)statsY }, 22, 1, BLUE); if (mod.def != 0) DrawTextEx(font, TextFormat("(補正 %+.1f)", mod.def), { (float)x + 250, (float)statsY }, 18, 1, ORANGE); statsY += lineH; }
    if (totalHp != 0) { DrawTextEx(font, TextFormat("最大HP : %+.0f", totalHp), { (float)x + 40, (float)statsY }, 22, 1, GREEN); if (mod.hp != 0) DrawTextEx(font, TextFormat("(補正 %+.0f)", mod.hp), { (float)x + 250, (float)statsY }, 18, 1, ORANGE); statsY += lineH; }
    if (totalSpd != 0) { DrawTextEx(font, TextFormat("速度   : %+.2f", totalSpd), { (float)x + 40, (float)statsY }, 22, 1, SKYBLUE); if (mod.spd != 0) DrawTextEx(font, TextFormat("(補正 %+.2f)", mod.spd), { (float)x + 250, (float)statsY }, 18, 1, ORANGE); statsY += lineH; }
    if (focusingItem.heal > 0) { DrawTextEx(font, TextFormat("回復量 : %.0f", focusingItem.heal), { (float)x + 40, (float)statsY }, 22, 1, PINK); statsY += lineH; }
    ...
    DrawTextEx(font, "閉じる", { closeBtn.x + closeBtn.width / 2 - txtSz.x / 2, closeBtn.y + closeBtn.height / 2 - txtSz.y / 2 }, 24, 1, inputEnabled ? WHITE : GRAY);
...
int UI::DrawTitleScreen(Font font) {
    ...
    if (deleteConfirmSlot > 0) {
        DrawRectangle(0, 0, sw, sh, Fade(BLACK, 0.8f)); // 背景を暗く
        int w = 500, h = 250;
        ...
        DrawTextEx(font, TextFormat("Slot %d のデータを削除しますか？", deleteConfirmSlot), { (float)x + 40, (float)y + 60 }, 24, 1, WHITE);
        DrawTextEx(font, "削除すると元に戻せません。", { (float)x + 80, (float)y + 100 }, 20, 1, RED);

        // YES
        if (DrawButton({ (float)x + 50, (float)y + 160, 150, 50 }, "削除する", font, RED)) {
            DataManager::DeleteSave(deleteConfirmSlot);
            deleteConfirmSlot = 0;
        }
        // NO
        if (DrawButton({ (float)x + 300, (float)y + 160, 150, 50 }, "キャンセル", font, GRAY)) {
            deleteConfirmSlot = 0;
        }
        return 0; // スロット選択は無効
    }
...
=======
        if (h.exists) {
            label = TextFormat("Slot %d: Lv.%d  Floor %d", i, h.playerLevel, h.floor); c = DARKGREEN;
        }
        else {
            label = TextFormat("Slot %d: (NO DATA)", i); c = DARKGRAY;
        }
        ...
        if (h.exists) {
            ...
            if (DrawButton(delBtn, "削除", font, MAROON)) {
                deleteConfirmSlot = i;
            }
>>>>>>> sub
        }
...
=======
        ...
        DrawTextEx(font, "右クリックドラッグでマップ移動", { 120, 620 }, 16, 1, LIGHTGRAY);
...
void UI::DrawCraftingMenu(Player& p, Font font, bool& isOpen) {
    ...
    if (UI::DrawButton({ (float)sw - 160, 60, 100, 40 }, "閉じる", font, locked ? Fade(RED, 0.5f) : RED) && !locked) isOpen = false;
...
=======
            if (UI::DrawButton(createBtn, "作成", font, locked ? Fade(ORANGE, 0.5f) : ORANGE) && !locked) {
>>>>>>> sub
                p.gold -= r.cost;
                for (auto& m : r.materials) {
...
void UI::DrawStorage(Player& p, Font font, bool& isOpen, std::vector<ItemData>& sItems, std::vector<ItemData>& sEquip) {
    ...
    for (int i = 0; i < perP; i++) {
        ...
        if (UI::DrawButton({ 440, r.y, 110, 38 }, "預ける >>", font, locked ? Fade(BLUE, 0.5f) : BLUE) && !locked) {
...
=======
        ...
        if (UI::DrawButton({ 620, r.y, 110, 38 }, "<< 取出す", font, locked ? Fade(DARKGREEN, 0.5f) : DARKGREEN) && !locked) if (p.AddToInventory(sItems[idx])) sItems.erase(sItems.begin() + idx);
    }
    ...
    if (UI::DrawButton({ (float)sw - 160, 70, 100, 45 }, "閉じる", font, locked ? Fade(RED, 0.5f) : RED) && !locked) isOpen = false;
...
void UI::DrawReforgeMenu(Player& p, Font font, bool& isOpen) {
    ...
    int sw = GetScreenWidth(), sh = GetScreenHeight();
    DrawRectangle(50, 50, sw - 100, sh - 100, Fade(BLACK, 0.95f)); DrawRectangleLinesEx({ 50, 50, (float)sw - 100, (float)sh - 100 }, 3, GOLD);
    if (UI::DrawButton({ (float)sw - 160, 60, 100, 40 }, "閉じる", font, locked ? Fade(RED, 0.5f) : RED) && !locked) { isOpen = false; reforgeItemIdx = -1; }
...
=======
            if (DrawButton({ 520, 400, 150, 50 }, "リフォージ", font, locked ? Fade(GREEN, 0.5f) : GREEN) && !locked) { p.gold -= cost; item.modifierId = DataManager::GetRandomModifierId(); }
        }
        else {
...
int UI::DrawWarpMenu(int maxFloor, Font font, bool& isOpen) {
    ...
    if (UI::DrawButton({ (float)sw - 370, 115, 100, 40 }, "閉じる", font, locked ? Fade(RED, 0.5f) : RED) && !locked) { isOpen = false; return 0; }
>>>>>>> sub
