...
void Game::SaveCurrentSlot() {
    if (player) {
        DataManager::SaveGame(currentSlot, player, floor, maxReachedFloor, storageItems, storageEquip);
...
void Game::DrawDebugRoom() {
    // 【修正】BeginDrawing() を削除しました (呼び出し元で呼ばれているため)
    ...
    for (auto& e : enemies) {
        e.Draw(true, camera, font, camera.position);
    }
