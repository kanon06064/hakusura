...
expToNext(100), skillPoints(0), gold(0)
{
    ...
    ItemData s1 = DataManager::GetItemConfigCopy(0);   // 初期剣
    ItemData s2 = DataManager::GetItemConfigCopy(100); // 初期斧
...
Color Player::GetItemRarityColor(const ItemData& item) {
    ...
    else if (item.id >= 400 && item.id < 500) tier = 4; // レジェンダリー帯
...
void Player::RecalculateStats() {
    ...
    for (const auto& node : skillTree) {
        if (node.unlocked) { bAtk += node.atkAdd; bDef += node.defAdd; bHp += node.hpAdd; }
    }
    ...
    for (int i = 0; i < 5; i++) {
        if (equippedArmor[i].id != -1) {
            ...
            bHp += equippedArmor[i].hpBonus + m.hp;
            bDef += equippedArmor[i].defBonus + m.def;
            bAtk += equippedArmor[i].atkBonus + m.atk;
            bSpd += equippedArmor[i].speedBonus + m.spd;
        }
    }
...
void Player::InitSkillTree() {
    ...
    skillTree.push_back({ 6, T("SMASH"),   {640, 400}, {0}, false, 2, 0, 0, 0, SKILL_ACTIVE_SMASH, 5.0f });
    skillTree.push_back({ 10, T("DASH"), {880, 480}, {0}, false, 2, 0, 0, 0, SKILL_ACTIVE_DASH, 3.0f });
}

bool Player::IsSkillAvailable(int id) {
    for (auto& n : skillTree) {
        if (n.id == id) {
            if (n.unlocked) return false;
            if (n.reqIds.empty()) return true;
            for (int r : n.reqIds) for (auto& rn : skillTree) if (rn.id == r && rn.unlocked) return true;
        }
    }
    return false;
}
...
void Player::UnlockSkill(int id) {
    if (IsSkillAvailable(id)) {
        for (auto& n : skillTree) {
            if (n.id == id && skillPoints >= n.cost) {
                skillPoints -= n.cost; n.unlocked = true;
                AudioManager::PlaySE(SE_SKILL); RecalculateStats(); return;
            }
        }
    }
}
...
bool Player::AddToInventory(ItemData item) {
    if (item.type == "EQUIP" || item.type == "ARMOR") {
        if (inventoryEquip.size() >= MAX_EQUIP_INV) return false;
        inventoryEquip.push_back(item);
    }
    else {
        for (auto& i : inventoryItems) if (i.id == item.id && i.count < MAX_ITEM_STACK) { i.count++; return true; }
        if (inventoryItems.size() >= MAX_ITEM_TYPES) return false;
...
void Player::UseItem(int idx) {
    if (idx >= 0 && idx < (int)inventoryItems.size() && inventoryItems[idx].type == "CONSUMABLE") {
...
void Player::EquipWeapon(int invIdx, int slot) {
    ...
    equippedData[slot] = inventoryEquip[invIdx];
    equippedWeapons[slot] = (WeaponType)equippedData[slot].weaponSubtype;
...
void Player::UnequipWeapon(int slot) {
    if (equippedData[slot].id == -1) return;
...
void Player::UnequipArmor(int slot) {
    if (equippedArmor[slot].id == -1) return;
...
// --- クエスト ---
void Player::UpdateHuntQuest(int eId) {
    for (auto& q : activeQuests) if (!q.isCompleted) {
        QuestData d = DataManager::GetQuestData(q.questId);
        if (d.type == QUEST_HUNT && d.targetId == eId) if (++q.currentCount >= d.targetCount) q.isCompleted = true;
    }
}

bool Player::CheckGatherQuest(int itemId, int count) {
    int sum = 0; for (auto& i : inventoryItems) if (i.id == itemId) sum += i.count;
    return sum >= count;
}

void Player::CompleteQuest(int qId) {
    for (auto it = activeQuests.begin(); it != activeQuests.end(); ++it) if (it->questId == qId) {
        QuestData d = DataManager::GetQuestData(qId);
        gold += d.rewardGold;
        if (d.rewardItemId != -1) {
            ItemData r = DataManager::GetItemConfigCopy(d.rewardItemId);
            r.count = d.rewardItemCount; AddToInventory(r);
        }
        clearedQuests.push_back(qId); activeQuests.erase(it);
        AudioManager::PlaySE(SE_REFORGE); return;
    }
}
...
void Player::PerformSmash(Vector3 ad, std::vector<Enemy>& enemies, Dungeon& d, EffectManager& fx) {
    ...
    for (auto& e : enemies) if (Vector3Distance(e.position, position) < 5.0f) {
        int dmg = (int)(attackPower * 2.5f); e.hp -= dmg; e.ApplyKnockback(ad, 3.0f, d);
        fx.SpawnDamageText(e.position, dmg); isStealth = false;
    }
}
...
void Player::Update(Camera3D& cam, Dungeon& d, std::vector<Enemy>& enemies, EffectManager& fx, bool stop) {
    ...
    // スキル入力
    if (IsKeyPressed(KEY_ONE) && IsSkillUnlocked(SKILL_ACTIVE_DASH) && dashCooldownTimer <= 0) { dashTimer = 0.2f; dashCooldownTimer = 3.0f; AudioManager::PlaySE(SE_SKILL); }
    ...
    if (IsKeyPressed(KEY_THREE) && IsSkillUnlocked(SKILL_ACTIVE_STEALTH) && stealthCooldownTimer <= 0) { isStealth = true; stealthTimer = 10.0f; stealthCooldownTimer = 15.0f; AudioManager::PlaySE(SE_SKILL); }
    ...
    float curSpd = (dashTimer > 0) ? baseSpeed * 2.5f : baseSpeed;
    ...
    // --- 状態遷移 (0:Atk, 3:Die, 4:Idle, 5:Sprint, 6:Walk) ---
    int targetAnim = 4; // Idle
    if (hp <= 0) targetAnim = 3;
    else if (attackTimer > 0) targetAnim = (currentWeapon == SWORD ? 0 : (currentWeapon == AXE ? 1 : 2));
    else if (dashTimer > 0) targetAnim = 5;
    else if (isMoving) targetAnim = 6;

    // ポーズのワープ防止: グループ遷移(4,5,6)間では時間をリセットしない
    ...
    // 再生速度を移動速度に同期
    float pSpd = 30.0f;
    if (currentAnimIndex == 6) pSpd = 30.0f * (curSpd / 0.18f);      // Walk
    else if (currentAnimIndex == 5) pSpd = 30.0f * (curSpd / 0.45f); // Sprint
    animTime += dt * pSpd;
}
...
void Player::Draw(bool debug) {
    ...
    DrawModel(gm.model, position, scale, (isStealth ? Fade(BLUE, 0.4f) : WHITE));

    // 武器のアタッチメント (レジェンダリー自動解決)
    if (currentWeapon != NONE && !isDead) {
        ...
        if (handIdx != -1) {
            ...
            Matrix playerWorld = MatrixMultiply(MatrixMultiply(MatrixScale(scale, scale, scale), gm.model.transform), MatrixTranslate(position.x, position.y, position.z));
            ...
            // レジェンダリー(400番台)用特殊モデル検索
            if (equipId >= 400 && equipId < 500) {
                std::string legendKey = baseKey + "_Legend";
                if (DataManager::loadedModels.count(legendKey)) finalKey = legendKey;
            }
            // アイテムID専用モデルがあれば優先
