...
bool Player::IsSkillAvailable(int id) { for (auto& n : skillTree) if (n.id == id) { if (n.unlocked) return false; if (n.reqIds.empty()) return true; for (int r : n.reqIds) for (auto& rn : skillTree) if (rn.id == r && rn.unlocked) return true; } return false; }
void Player::UnlockSkill(int id) { if (IsSkillAvailable(id)) { for (auto& n : skillTree) if (n.id == id && skillPoints >= n.cost) { skillPoints -= n.cost; n.unlocked = true; AudioManager::PlaySE(SE_SKILL); RecalculateStats(); return; } } }
...
void Player::LevelUp(EffectManager& fx) {
    ...
    fx.SpawnDamageText(position, 999); AudioManager::PlaySE(SE_LEVELUP); RecalculateStats(); hp = maxHp;
...
void Player::EquipWeapon(int invIdx, int slot) { if (invIdx < 0 || invIdx >= (int)inventoryEquip.size()) return; if (equippedData[slot].id != -1) inventoryEquip.push_back(equippedData[slot]); equippedData[slot] = inventoryEquip[invIdx]; equippedWeapons[slot] = (WeaponType)equippedData[slot].weaponSubtype; inventoryEquip.erase(inventoryEquip.begin() + invIdx); if (activeSlot == slot) currentWeapon = equippedWeapons[slot]; AudioManager::PlaySE(SE_CLICK); RecalculateStats(); }
...
void Player::EquipArmor(int i, int slot) { if (i < 0 || i >= (int)inventoryEquip.size()) return; if (equippedArmor[slot].id != -1) inventoryEquip.push_back(equippedArmor[slot]); equippedArmor[slot] = inventoryEquip[i]; inventoryEquip.erase(inventoryEquip.begin() + i); AudioManager::PlaySE(SE_CLICK); RecalculateStats(); }
...
bool Player::CheckGatherQuest(int i, int c) { int s = 0; for (auto& it : inventoryItems) if (it.id == i) s += it.count; return s >= c; }
...
// --- メイン更新：新アニメーション(5:Sprint, 6:Walk)同期 ---
void Player::Update(Camera3D& cam, Dungeon& d, std::vector<Enemy>& enemies, EffectManager& fx, bool stop) {
    ...
    // --- 状態遷移 (4:Idle, 5:Sprint, 6:Walk) ---
    int targetAnim = 4; // Idle
    if (hp <= 0) targetAnim = 3; // Die
    else if (attackTimer > 0) targetAnim = (currentWeapon == SWORD ? 0 : (currentWeapon == AXE ? 1 : 2));
    else if (dashTimer > 0) targetAnim = 5; // Sprint (更新)
    else if (isMoving) targetAnim = 6;      // Walk (更新)

    // ループグループ(4,5,6)間では時間をリセットしない
    if (targetAnim != currentAnimIndex) {
        if (!(targetAnim >= 4 && currentAnimIndex >= 4)) animTime = 0;
...
void Player::Draw(bool debug) {
    ...
    for (int i = 0; i < gm.model.boneCount; i++) gm.model.bindPose[i].scale = { 1, 1, 1 };
