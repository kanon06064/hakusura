...
void Dungeon::Generate(bool homeMode) {
    ...
    if (homeMode) {
        ...
        stairsUpPos = { (float)rooms[0].x * TILE_SIZE + 1, 0.1f, (float)rooms[0].y * TILE_SIZE + 1 };
        stairsDownPos = { (float)rooms.back().x * TILE_SIZE + 1, 0.1f, (float)rooms.back().y * TILE_SIZE + 1 };
    }
}
...
void Dungeon::UpdateVisibility(Vector3 p) {
    if (isHome) return;
    int gx = (int)floorf((p.x + TILE_SIZE / 2) / TILE_SIZE), gz = (int)floorf((p.z + TILE_SIZE / 2) / TILE_SIZE);
...
bool Dungeon::IsWall(float x, float z) {
    int gx = (int)floorf((x + TILE_SIZE / 2) / TILE_SIZE), gz = (int)floorf((z + TILE_SIZE / 2) / TILE_SIZE);
...
Vector3 Dungeon::GetStartPosition() { return { (float)rooms[0].x * TILE_SIZE + 1, 0.5f, (float)rooms[0].y * TILE_SIZE + 1 }; }
